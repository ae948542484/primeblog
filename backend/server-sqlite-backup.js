...
async function initDB() {
  try {
    ...
    try {
      db.run('ALTER TABLE posts ADD COLUMN status TEXT DEFAULT "published"');
    } catch { /* ignore */ }
    try {
      db.run('ALTER TABLE posts ADD COLUMN slug TEXT DEFAULT ""');
    } catch { /* ignore */ }
    try {
      db.run('ALTER TABLE posts ADD COLUMN word_count INTEGER DEFAULT 0');
    } catch { /* ignore */ }
    try {
      db.run('ALTER TABLE posts ADD COLUMN reading_time INTEGER DEFAULT 0');
    } catch { /* ignore */ }
    try {
      db.run('ALTER TABLE posts ADD COLUMN updated_at DATETIME DEFAULT CURRENT_TIMESTAMP');
    } catch { /* ignore */ }
    
    // 为已有文章生成 slug
    try {
      const postsResult = db.exec('SELECT id, title FROM posts WHERE slug IS NULL OR slug = ""');
    ...
    // 修复已有文章的 status 字段（设置为 published）
    try {
      db.run('UPDATE posts SET status = "published" WHERE status IS NULL OR status = ""');
    } catch { /* ignore */ }
    
    // 升级 visitors 表结构
    try {
      db.run('ALTER TABLE visitors ADD COLUMN fingerprint TEXT');
    } catch { /* ignore - column may already exist */ }
    try {
      db.run('ALTER TABLE visitors ADD COLUMN online_status INTEGER DEFAULT 1');
    } catch { /* ignore - column may already exist */ }
    
    // 升级 tags 表结构
    try {
      db.run('ALTER TABLE tags ADD COLUMN slug TEXT');
    } catch { /* ignore - column may already exist */ }
    
    // 为已有标签生成 slug
    try {
      const tagsResult = db.exec('SELECT id, name FROM tags WHERE slug IS NULL OR slug = ""');
      const tags = queryToArray(tagsResult);
      tags.forEach(tag => {
        const slug = tag.name.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, '');
        db.run('UPDATE tags SET slug = ? WHERE id = ?', [slug, tag.id]);
      });
    } catch { /* ignore */ }
    ...
    console.log('📊 表结构: posts, columns, messages, images, authors, visitors, links, tags, post_tags');
